...
export type ClientMessage = Pick<Tables["mensagens"]["Row"],
  "id" | "cliente_id" | "sender" | "text" | "type" | "read_at" | "created_at" | "time" | "seq" | "doc_name" | "duration" | "transcricao"
>;
...
export async function loadClientsData(supabase: SupabaseClient<Database>): Promise<ClientsData> {
  const [clients, messages, triages, documents, history, guides] = await Promise.all([
    ...
    supabase.from("mensagens").select("id,cliente_id,sender,text,type,read_at,created_at,time,seq,doc_name,duration,transcricao").order("seq", { ascending: false }).limit(500),
