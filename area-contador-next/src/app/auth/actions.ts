...
import { registrarAtendimentoExpress } from "@/lib/pagamento";
...
// Só deixa excluir créditos que já saíram de circulação (usado/cancelado) —
// um crédito "ativo" precisa ser cancelado primeiro, senão o código some da
// lista mas continua resgatável via link por quem já tiver o link salvo.
export async function deleteServiceCredit(id:number){if(!Number.isInteger(id)||id<=0)return {ok:false as const,message:"Crédito inválido."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const {error}=await supabase.from("creditos").delete().eq("id",id).neq("status","ativo");if(error)return {ok:false as const,message:"Não foi possível excluir o crédito."};revalidatePath("/");return {ok:true as const,message:"Crédito excluído."};}

export type ParcelamentoManualInput={clientId:string;orgao:string;descricao:string;valorTotalCents:number;numeroParcelas:number;valorParcelaCents:number;diaVencimento:number|null;observacoes:string;tributo:string;dataInicio:string};
export async function createParcelamentoManual(input:ParcelamentoManualInput){const orgao=input.orgao.trim().slice(0,120);if(!input.clientId||orgao.length<2)return {ok:false as const,message:"Informe o cliente e o órgão do parcelamento."};const numeroParcelas=Math.max(1,Math.round(Number(input.numeroParcelas))||1);const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const admin=adminClient();if(!admin)return {ok:false as const,message:"Serviço indisponível."};const {data,error}=await admin.from("parcelamentos_manuais").insert({cliente_ref:input.clientId,orgao,descricao:input.descricao.trim().slice(0,300)||null,valor_total_cents:Math.max(0,Math.round(Number(input.valorTotalCents))||0),numero_parcelas:numeroParcelas,valor_parcela_cents:Math.max(0,Math.round(Number(input.valorParcelaCents))||0),dia_vencimento:input.diaVencimento?Math.min(28,Math.max(1,Math.round(input.diaVencimento))):null,observacoes:input.observacoes.trim().slice(0,2000)||null,tributo:input.tributo.trim().slice(0,80)||null,data_inicio:input.dataInicio||null,created_by:userId}).select("*").single();if(error||!data){await registrarErro(admin,{origem:"createParcelamentoManual",codigo:"insert_falhou",mensagem:error?.message||"sem dados retornados",rota:"actions/createParcelamentoManual",severidade:"erro",contexto:{clientId:input.clientId}});return {ok:false as const,message:"Não foi possível registrar o parcelamento."};}revalidatePath("/");return {ok:true as const,data,message:"Parcelamento registrado."};}
export async function updateRecurrenceLabel(clientId:string,tipo:string){const label=tipo.trim().slice(0,120);if(!clientId||label.length<2)return {ok:false as const,message:"Informe o tipo da recorrência."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const {data,error}=await supabase.from("clientes").update({recorrente_tipo:label}).eq("id",clientId).select("*").single();if(error||!data)return {ok:false as const,message:"Não foi possível atualizar o tipo da recorrência."};revalidatePath("/");return {ok:true as const,data,message:"Tipo da recorrência atualizado."};}
export async function marcarParcelaPaga(id:number){if(!Number.isInteger(id)||id<=0)return {ok:false as const,message:"Parcelamento inválido."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const admin=adminClient();if(!admin)return {ok:false as const,message:"Serviço indisponível."};const {data:row}=await admin.from("parcelamentos_manuais").select("parcelas_pagas,numero_parcelas").eq("id",id).maybeSingle();if(!row)return {ok:false as const,message:"Parcelamento não encontrado."};const parcelasPagas=Math.min(row.numero_parcelas,row.parcelas_pagas+1);const status=parcelasPagas>=row.numero_parcelas?"quitado":"ativo";const {data,error}=await admin.from("parcelamentos_manuais").update({parcelas_pagas:parcelasPagas,status,updated_at:new Date().toISOString()}).eq("id",id).select("*").single();if(error||!data){await registrarErro(admin,{origem:"marcarParcelaPaga",codigo:"update_falhou",mensagem:error?.message||"sem dados retornados",rota:"actions/marcarParcelaPaga",severidade:"erro",contexto:{id}});return {ok:false as const,message:"Não foi possível atualizar o parcelamento."};}revalidatePath("/");return {ok:true as const,data,message:status==="quitado"?"Parcelamento quitado.":"Parcela marcada como paga."};}
export async function atualizarParcelamentoManual(id:number,patch:{status?:string;observacoes?:string}){if(!Number.isInteger(id)||id<=0)return {ok:false as const,message:"Parcelamento inválido."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const admin=adminClient();if(!admin)return {ok:false as const,message:"Serviço indisponível."};const update:{updated_at:string;status?:string;observacoes?:string|null}={updated_at:new Date().toISOString()};if(patch.status)update.status=patch.status.trim().slice(0,40);if(typeof patch.observacoes==="string")update.observacoes=patch.observacoes.trim().slice(0,2000)||null;const {data,error}=await admin.from("parcelamentos_manuais").update(update).eq("id",id).select("*").single();if(error||!data){await registrarErro(admin,{origem:"atualizarParcelamentoManual",codigo:"update_falhou",mensagem:error?.message||"sem dados retornados",rota:"actions/atualizarParcelamentoManual",severidade:"erro",contexto:{id}});return {ok:false as const,message:"Não foi possível atualizar o parcelamento."};}revalidatePath("/");return {ok:true as const,data,message:"Parcelamento atualizado."};}
export async function deleteParcelamentoManual(id:number){if(!Number.isInteger(id)||id<=0)return {ok:false as const,message:"Parcelamento inválido."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const admin=adminClient();if(!admin)return {ok:false as const,message:"Serviço indisponível."};const {error}=await admin.from("parcelamentos_manuais").delete().eq("id",id);if(error){await registrarErro(admin,{origem:"deleteParcelamentoManual",codigo:"delete_falhou",mensagem:error.message,rota:"actions/deleteParcelamentoManual",severidade:"erro",contexto:{id}});return {ok:false as const,message:"Não foi possível excluir o parcelamento."};}revalidatePath("/");return {ok:true as const,message:"Parcelamento excluído."};}
function dataParcelaBR(dataInicio:string,n:number):string{const [ano,mes,dia]=dataInicio.split("-").map(Number);return new Date(ano,mes-1+n,dia).toLocaleDateString("pt-BR");}
export async function avisarParcela(parcelamentoId:number,numeroParcela:number){if(!Number.isInteger(parcelamentoId)||parcelamentoId<=0||!Number.isInteger(numeroParcela)||numeroParcela<=0)return {ok:false as const,message:"Parcela inválida."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const admin=adminClient();if(!admin)return {ok:false as const,message:"Serviço indisponível."};const {data:parcelamento}=await admin.from("parcelamentos_manuais").select("*").eq("id",parcelamentoId).maybeSingle();if(!parcelamento)return {ok:false as const,message:"Parcelamento não encontrado."};const {data:cliente}=await admin.from("clientes").select("name,email,phone").eq("id",parcelamento.cliente_ref).maybeSingle();if(!cliente)return {ok:false as const,message:"Cliente não encontrado."};if(!cliente.email&&!cliente.phone)return {ok:false as const,message:"Cliente não tem e-mail nem telefone cadastrado."};const valor=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format((parcelamento.valor_parcela_cents||0)/100);const vencimento=parcelamento.data_inicio?dataParcelaBR(parcelamento.data_inicio,numeroParcela-1):"a combinar";const assunto=`Parcela ${numeroParcela}/${parcelamento.numero_parcelas} — ${parcelamento.orgao}`;const mensagem=`Lembrete da parcela <strong>${numeroParcela}/${parcelamento.numero_parcelas}</strong> do parcelamento${parcelamento.tributo?` de ${parcelamento.tributo}`:""} (${parcelamento.orgao}), no valor de <strong>${valor}</strong>, com vencimento em <strong>${vencimento}</strong>.`;const resultado=await notify.notifyCliente({name:cliente.name,email:cliente.email,phone:cliente.phone},assunto,mensagem);const jaAvisadas=new Set<number>(parcelamento.avisos_enviados||[]);jaAvisadas.add(numeroParcela);const {error}=await admin.from("parcelamentos_manuais").update({avisos_enviados:Array.from(jaAvisadas),updated_at:new Date().toISOString()}).eq("id",parcelamentoId);if(error){await registrarErro(admin,{origem:"avisarParcela",codigo:"update_falhou",mensagem:error.message,rota:"actions/avisarParcela",severidade:"erro",contexto:{parcelamentoId,numeroParcela}});}revalidatePath("/");const canais:string[]=[];const r=resultado as {email?:{ok?:boolean};whatsapp?:unknown};if(r.email?.ok)canais.push("e-mail");if(r.whatsapp)canais.push("WhatsApp");return {ok:true as const,message:canais.length?`Aviso enviado por ${canais.join(" e ")}.`:"Nenhum canal disponível (e-mail/WhatsApp não configurado ou não cadastrado pro cliente)."};}
...
export async function saveClientDossier(input:ClientDossierInput){
  ...
  if(input.cpf.replace(/\D/g,"")&&!validarCpfCnpj(input.cpf).valido)return {ok:false as const,message:"CPF/CNPJ inválido — confira os números digitados."};
  if(input.phone.replace(/\D/g,"")&&!validarTelefone(input.phone))return {ok:false as const,message:"Telefone inválido — confira o número digitado."};
  const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};
...
// Cadastra o cliente e já entra na fila (status "waiting"), sem passar pelo
// checkout público de pagamento — pensado pra casos combinados por telefone,
// presencial etc. Express/recorrente reaproveitam registrarAtendimentoExpress
// (mesma lógica de prazo/tarefa do checkout pago), criando por baixo dos
// panos uma cobrança "cortesia" (valor_cents:0) só pra satisfazer a
// constraint NOT NULL de atendimentos_express.cobranca_id — não gera
// cobrança de verdade nenhuma no Asaas.
export type NovoAtendimentoInput = {
  name: string;
  cpf: string;
  email: string;
  phone: string;
  modalidade: "express" | "agendado" | "recorrente";
  date?: string;
  time?: string;
  recorrenteTipo?: string;
};
export async function createAtendimentoSemCheckout(input: NovoAtendimentoInput) {
  const name = input.name.trim().replace(/\s+/g, " ").slice(0, 120);
  const cpf = input.cpf.replace(/[^\d]/g, "").slice(0, 14);
  if (name.length < 2) return { ok: false as const, message: "Informe o nome do cliente." };
  if (cpf && !validarCpfCnpj(cpf).valido) {
    return { ok: false as const, message: "CPF/CNPJ inválido — confira os números digitados." };
  }
  if (input.phone.replace(/\D/g, "") && !validarTelefone(input.phone)) {
    return { ok: false as const, message: "Telefone inválido — confira o número digitado." };
  }
  if (input.modalidade === "agendado" && (!input.date || !input.time)) {
    return { ok: false as const, message: "Informe data e horário do agendamento." };
  }
  const supabase = await createClient();
  if (!supabase) return { ok: false as const, message: "Conexão indisponível." };
  const { data: claims } = await supabase.auth.getClaims();
  ...
  if (!staff) return { ok: false as const, message: "Ação não autorizada." };
  if (cpf) {
    const { data: duplicate } = await supabase.from("clientes").select("id").eq("cpf", cpf).maybeSingle();
    if (duplicate) return { ok: false as const, message: "Já existe um cliente com este CPF/CNPJ." };
  }
  if (input.modalidade === "agendado") {
    const { data: conflict } = await supabase.from("agendamentos").select("id").eq("date", input.date!).eq("time", input.time!).neq("status", "done").limit(1).maybeSingle();
    if (conflict) return { ok: false as const, message: "Este horário já está ocupado." };
  }

  const admin = adminClient();
  if (!admin) return { ok: false as const, message: "Serviço indisponível." };
  ...
  const { data: cliente, error: clienteError } = await admin
    ...
    .insert({
      ...
      evidences: [],
      perfil_operacional: {},
      atendimento_modalidade: input.modalidade === "agendado" ? "agendado" : "sem_agendamento",
      recorrente: input.modalidade === "recorrente" ? true : null,
      recorrente_tipo: input.modalidade === "recorrente" ? input.recorrenteTipo?.trim().slice(0, 120) || "Acompanhamento mensal" : null,
    })
    .select("*")
    .single();
  if (clienteError || !cliente) return { ok: false as const, message: "Não foi possível cadastrar o cliente." };

  let protocolo: string | null = null;

  if (input.modalidade === "express" || input.modalidade === "recorrente") {
    const { data: cobranca, error: cobrancaError } = await admin
      .from("cobrancas")
      .insert({ cliente_ref: id, origem: "manual_staff", modalidade: "sem_agendamento", status: "paid", valor_cents: 0, paid_at: new Date().toISOString() })
      .select("id")
      .single();
    if (cobrancaError || !cobranca) return { ok: false as const, message: "Cliente criado, mas não foi possível iniciar o atendimento." };
    await registrarAtendimentoExpress(admin, {
      cobrancaId: cobranca.id,
      clienteRef: id,
      assunto: input.modalidade === "recorrente" ? "Atendimento recorrente" : "Atendimento Express",
    });
    const { data: atendimento } = await admin.from("atendimentos_express").select("id").eq("cobranca_id", cobranca.id).maybeSingle();
    if (atendimento) protocolo = protocoloAtendimento("express", atendimento.id);
  } else {
    const { data: agendamento, error: agendamentoError } = await admin
      .from("agendamentos")
      .insert({ cliente_ref: id, client_name: name, date: input.date!, time: input.time!, status: "pending" })
      .select("id")
      .single();
    if (agendamentoError || !agendamento) return { ok: false as const, message: "Cliente criado, mas não foi possível agendar o atendimento." };
    protocolo = protocoloAtendimento("agendado", agendamento.id);
  }

  revalidatePath("/");
  return { ok: true as const, data: cliente, protocolo, message: `Cliente cadastrado e atendimento iniciado${protocolo ? ` — protocolo ${protocolo}` : ""}.` };
}
...
const configurableKeys=new Set(["timer_config","radar_fiscal_config","radar_fiscal_clientes","triagem_config","triagem_regras","triagem_assuntos","processos_servicos","nfse_config","nota_fiscal_config","ia_skills","chat_appearance","chat_shortcuts","copilot_shortcuts","area_cliente_config","painel_preferencias","tarefas","hero_banner_config"]);
