...
export async function marcarParcelaPaga(id:number){if(!Number.isInteger(id)||id<=0)return {ok:false as const,message:"Parcelamento inválido."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const {data:row}=await supabase.from("parcelamentos_manuais").select("parcelas_pagas,numero_parcelas").eq("id",id).maybeSingle();if(!row)return {ok:false as const,message:"Parcelamento não encontrado."};const parcelasPagas=Math.min(row.numero_parcelas,row.parcelas_pagas+1);const status=parcelasPagas>=row.numero_parcelas?"quitado":"ativo";const {data,error}=await supabase.from("parcelamentos_manuais").update({parcelas_pagas:parcelasPagas,status,updated_at:new Date().toISOString()}).eq("id",id).select("*").single();if(error||!data)return {ok:false as const,message:"Não foi possível atualizar o parcelamento."};revalidatePath("/");return {ok:true as const,data,message:status==="quitado"?"Parcelamento quitado.":"Parcela marcada como paga."};}
export async function atualizarParcelamentoManual(id:number,patch:{status?:string;observacoes?:string}){if(!Number.isInteger(id)||id<=0)return {ok:false as const,message:"Parcelamento inválido."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const update:{updated_at:string;status?:string;observacoes?:string|null}={updated_at:new Date().toISOString()};if(patch.status)update.status=patch.status.trim().slice(0,40);if(typeof patch.observacoes==="string")update.observacoes=patch.observacoes.trim().slice(0,2000)||null;const {data,error}=await supabase.from("parcelamentos_manuais").update(update as any).eq("id",id).select("*").single();if(error||!data)return {ok:false as const,message:"Não foi possível atualizar o parcelamento."};revalidatePath("/");return {ok:true as const,data,message:"Parcelamento atualizado."};}
export async function deleteParcelamentoManual(id:number){if(!Number.isInteger(id)||id<=0)return {ok:false as const,message:"Parcelamento inválido."};const supabase=await createClient();if(!supabase)return {ok:false as const,message:"Conexão indisponível."};const {data:claims}=await supabase.auth.getClaims();const userId=claims?.claims?.sub;if(!userId)return {ok:false as const,message:"Sessão expirada."};const {data:staff}=await supabase.from("staff").select("id").eq("id",userId).maybeSingle();if(!staff)return {ok:false as const,message:"Ação não autorizada."};const {error}=await supabase.from("parcelamentos_manuais").delete().eq("id",id);if(error)return {ok:false as const,message:"Não foi possível excluir o parcelamento."};revalidatePath("/");return {ok:true as const,message:"Parcelamento excluído."};}
