// Relatório do cliente: template único usado pelo painel do contador e pela
// Área do Cliente. O relatório de atendimento atesta o caso e sua resolução;
// o relatório de pendências é emitido separadamente quando necessário.
window.OCRelatorio = (function () {
  ...
  function temaDo(rel) {
    return rel && rel.tipoRelatorio === 'pendencias' ? '#111315' : PINE;
  }

  function temaRgb(rel) {
    return rel && rel.tipoRelatorio === 'pendencias' ? [17, 19, 21] : [10, 49, 33];
  }
  ...
  function linhas(texto) {
    return String(texto || '').split('\n')
      .map(function (linha) { return linha.replace(/^\s*[-•*]\s*/, '').trim(); })
      .filter(Boolean);
  }

  function itens(texto, compacto) {
    var lista = linhas(texto);
    if (!lista.length) return '<p style="margin:0;color:#66736D;">—</p>';
    return '<ul style="margin:0;padding-left:18px;">' + lista.map(function (linha) {
      return '<li style="margin:0 0 ' + (compacto ? '4' : '6') + 'px;line-height:1.45;">' + esc(linha) + '</li>';
    }).join('') + '</ul>';
  }
  ...
  function textoEmParagrafos(texto) {
    ...
    return partes.map(function (parte) {
      return '<p style="margin:0 0 6px;line-height:1.5;">' + esc(parte) + '</p>';
    }).join('');
  }

  function dataBR(valor, comHora) {
  ...
  function protocoloDo(rel) {
    var id = /^\d+$/.test(String(rel.id || '')) ? String(rel.id).padStart(6, '0') : 'PREVIA';
    return 'OC-R' + id + '-V' + String(rel.versao || 1);
  }

  function urlValidacao(rel) {
    if (!rel.codigoValidacao || typeof window === 'undefined') return '';
    var url = new URL('/validar-relatorio.html', window.location.origin);
    url.searchParams.set('codigo', rel.codigoValidacao);
    return url.toString();
  }

  function qrCodeDataURL(rel) {
    var url = urlValidacao(rel);
    if (!url || typeof qrcode !== 'function' || typeof document === 'undefined') return '';
    try {
      var qr = qrcode(0, 'M');
      qr.addData(url);
      qr.make();
      ...
      for (var linha = 0; linha < modulos; linha++) {
        for (var coluna = 0; coluna < modulos; coluna++) {
  ...
  function qrCodeHTML(rel) {
    var imagem = qrCodeDataURL(rel);
    if (imagem) return '<img alt="QR Code de validação" src="' + esc(imagem) + '" ' +
      'style="width:62px;height:62px;display:block;image-rendering:pixelated;" />';
    return '<div style="width:62px;height:62px;border:1px solid #CBD2CE;display:flex;align-items:center;' +
      'justify-content:center;text-align:center;font-size:7px;line-height:1.25;color:#66736D;background:#fff;">' +
      (rel.codigoValidacao ? 'CÓDIGO<br>DE VALIDAÇÃO' : 'DISPONÍVEL<br>APÓS EMISSÃO') + '</div>';
  }

  function secao(titulo, corpo, compacto, destaque, tema) {
    return '<section style="break-inside:avoid;margin-bottom:' + (compacto ? '12' : '18') + 'px;' +
      (destaque ? 'background:' + (tema === '#111315' ? '#F2F2F2' : '#F2F7F4') + ';border-left:3px solid ' + tema + ';padding:12px 14px;border-radius:0 9px 9px 0;' : '') + '">' +
      '<div style="font-size:10px;font-weight:800;letter-spacing:.65px;text-transform:uppercase;color:' +
        tema + ';margin-bottom:6px;">' + esc(titulo) + '</div>' +
      '<div style="font-size:' + (compacto ? '12.5' : '13') + 'px;color:' + TINTA + ';line-height:1.5;">' + corpo + '</div>' +
    '</section>';
  }

  function corpoAtendimento(rel) {
    var tema = temaDo(rel);
    return secao('Descrição do caso', textoEmParagrafos(rel.problema), true, false, tema) +
  ...
  function corpoPendencias(rel) {
    var tema = temaDo(rel);
    var origem = '<p style="margin:0;line-height:1.5;">Este relatório foi elaborado com base no Relatório de Pendências da Receita Federal e nas demais evidências levantadas durante o atendimento.</p>';
    return secao('Origem e escopo da análise', origem + textoEmParagrafos(rel.problema), true, true, tema) +
  ...
  function montarNo(rel, cliente) {
    rel = rel || {};
    cliente = cliente || {};
    var tipoRelatorio = rel.tipoRelatorio === 'pendencias' ? 'pendencias' : 'atendimento';
    var nome = String(rel.clienteNome || cliente.name || rel.clientRef || 'Cliente').trim().replace(/\s+/g, ' ');
    var documento = formatarDocumento(rel.clienteCpf || cliente.cpf || '');
    var documentoLabel = String(documento).replace(/\D/g, '').length === 14 ? 'CNPJ' : 'CPF';
    var contador = String(rel.contadorNome || 'Contador responsável').trim();
    var crc = String(rel.contadorCrc || '').replace(/^\s*crc\s*/i, '').trim();
    var protocolo = protocoloDo(rel);
    var assinatura = rel.contadorAssinatura || '';
    // A marca do relatório é institucional e não muda conforme o perfil do contador.
    // O SVG pinho oficial recebe o acabamento creme no cabeçalho escuro.
    var logo = '/logo.svg';
    var tema = temaDo(rel);

    var el = document.createElement('div');
    el.style.cssText = 'width:720px;background:' + PAPER + ';color:' + TINTA +
      ';font-family:\'Outfit\',Arial,sans-serif;box-sizing:border-box;';
    el.innerHTML =
      '<header style="background:' + tema + ';color:#fff;padding:22px 34px 42px;position:relative;overflow:hidden;' +
        ...
        '<div style="display:flex;align-items:center;gap:10px;position:relative;z-index:1;">' +
          ...
          '<div style="font-size:9px;opacity:.82;margin-top:4px;letter-spacing:.35px;">Seu contador pessoal.</div></div>' +
        '</div>' +
        '<div style="text-align:right;font-size:9.5px;line-height:1.5;position:relative;z-index:1;">' +
          '<div style="font-weight:800;letter-spacing:.8px;">' +
            (tipoRelatorio === 'pendencias' ? 'RELATÓRIO DE PENDÊNCIAS' : 'RELATÓRIO DE ATENDIMENTO') + '</div>' +
          '<div>Emitido em ' + dataBR(rel.entregueEm || rel.createdAt, true) + '</div>' +
          '<div>' + esc(protocolo) + '</div>' +
        '</div>' +
        '<svg viewBox="0 0 720 44" preserveAspectRatio="none" style="position:absolute;left:0;bottom:-1px;width:100%;height:44px;z-index:2;">' +
          '<path d="M0,20 C150,42 415,4 720,22 L720,44 L0,44 Z" fill="#FFFFFF"/></svg>' +
      '</header>' +

      '<main style="padding:14px 34px 16px;">' +
        '<div style="display:grid;grid-template-columns:minmax(0,1fr) 180px;gap:16px;background:' + SUAVE +
          ';border-radius:8px;padding:12px 15px;margin-bottom:16px;break-inside:avoid;">' +
          ...
          (documento ? '<div style="text-align:right;"><div style="font-size:9px;text-transform:uppercase;letter-spacing:.6px;color:#748079;">' + documentoLabel + '</div>' +
            '<div style="font-size:14px;font-weight:750;white-space:nowrap;">' + esc(documento) + '</div></div>' : '') +
        '</div>' +
        '<h1 style="font-size:18px;color:' + tema +
          ';margin:0 0 15px;line-height:1.25;">' + esc(rel.titulo || 'Relatório de atendimento') + '</h1>' +
        ...
        '<section style="break-inside:avoid;margin-top:20px;text-align:center;">' +
          '<div style="width:270px;margin:0 auto;">' +
            (assinatura ? '<img src="' + esc(assinatura) + '" alt="Assinatura" style="height:48px;max-width:240px;object-fit:contain;display:block;margin:0 auto -2px;" />' :
      ...
      '<footer class="oc-report-validation" style="break-inside:avoid;background:#F3F1EA;border-top:2px solid ' + tema + ';padding:11px 34px;' +
        ...
        '<div style="flex:1;min-width:0;">' +
          ...
          '<div style="font-size:8.5px;margin-top:4px;overflow-wrap:anywhere;">Código: <strong>' + esc(rel.codigoValidacao || 'gerado na emissão') + '</strong></div>' +
        '</div>' +
        '<div style="text-align:right;font-size:8.8px;line-height:1.45;white-space:nowrap;">' +
          '<strong style="color:' + tema + ';">' + esc(protocolo) + '</strong><br>Versão ' + esc(rel.versao || 1) +
          '<br>' + dataBR(rel.entregueEm || rel.createdAt) + '</div>' +
      '</footer>';
    return el;
  }

  function montarHTML(rel, cliente) {
    return montarNo(rel, cliente).outerHTML;
  }

  function adicionarRodapePdf(pdf, rel) {
    var total = pdf.internal.getNumberOfPages();
    var qrImagem = qrCodeDataURL(rel);
    var protocolo = protocoloDo(rel);
    var codigo = rel.codigoValidacao || 'gerado na emissão';
    var rgbTema = temaRgb(rel);
    for (var pagina = 1; pagina <= total; pagina++) {
      ...
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(5.8);
      var aviso = 'O QR Code confirma emissão, autoria, versão e integridade deste registro. As informações refletem o atendimento na data de emissão; comprovantes oficiais permanecem a fonte para atos de órgãos públicos ou terceiros.';
      pdf.text(pdf.splitTextToSize(aviso, 119), 30, 276.1, { lineHeightFactor: 1.15 });
      pdf.setFontSize(5.5);
      pdf.text('Código: ' + String(codigo), 30, 285.4);
      ...
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(5.7);
      pdf.text('Versão ' + String(rel.versao || 1), 198, 276.3, { align: 'right' });
  ...
  function baixarPDF(rel, cliente) {
    if (typeof html2pdf === 'undefined') {
      alert('Gerador de PDF ainda carregando. Tente novamente em um instante.');
      return Promise.resolve(false);
    }
    var no = montarNo(rel, cliente);
    var rodapeHtml = no.querySelector('.oc-report-validation');
    ...
    var nomeCliente = (rel && rel.clienteNome) || (cliente && cliente.name) || rel.clientRef || 'cliente';
    var nomeArq = 'Relatorio-OlaContador-' + String(nomeCliente)
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-zA-Z0-9]+/g, '-') + '.pdf';
    var imagens = Array.prototype.slice.call(no.querySelectorAll('img'));
    var prontas = Promise.all(imagens.map(function (img) {
      if (img.complete && img.naturalWidth) return Promise.resolve();
      if (img.decode) return img.decode().catch(function () {});
      return new Promise(function (resolve) { img.onload = img.onerror = resolve; });
    }));

    return prontas.then(function () {
      ...
      }).from(no).toCanvas().toPdf();
      return worker.get('canvas').then(function (canvas) {
        return worker.get('pdf').then(function (pdfBase) {
          var PdfCtor = pdfBase.constructor;
          var pdf = new PdfCtor({ unit: 'mm', format: 'a4', orientation: 'portrait' });
          var larguraMaxima = 194;
          var alturaMaxima = 256;
          var largura = larguraMaxima;
