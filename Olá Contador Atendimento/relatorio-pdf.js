...
window.OCRelatorio = (function () {
  ...
  function montarNo(rel, cliente) {
    ...
    // Mesma fonte do resto do produto (--font-title/--font-body no styles.css):
    // um relatório em Georgia/serifada destoava de tudo — nada no app usa serifa.
    ...
    el.innerHTML =
      // ---------- Cabeçalho com a marca, fechado por um arco (não uma borda reta) ----------
      '<div style="background:' + PINE + ';color:#fff;padding:26px 40px 40px;display:flex;justify-content:space-between;align-items:center;position:relative;overflow:hidden;">' +
        '<div style="display:flex;align-items:center;gap:11px;">' +
          marca +
          '<span style="font-weight:700;font-size:20px;">Olá, Contador' +
            '<span style="color:' + CORAL + ';">.</span></span>' +
        '</div>' +
        '<div style="text-align:right;font-size:10.5px;line-height:1.5;opacity:.9;">' +
        ...
        '</div>' +
        '<svg viewBox="0 0 720 32" preserveAspectRatio="none" style="position:absolute;left:0;bottom:-1px;width:100%;height:32px;display:block;">' +
          '<path d="M0,0 Q360,32 720,0 L720,32 L0,32 Z" fill="' + PAPER + '"/>' +
        '</svg>' +
      '</div>' +
      ...
      '<div style="padding:16px 40px 24px;">' +
        ...
        '<div style="display:flex;justify-content:space-between;gap:16px;background:#F7F5F0;border-radius:8px;padding:14px 18px;margin-bottom:24px;">' +
          '<div><div style="font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:#888;">Cliente</div>' +
            '<div style="font-size:15px;font-weight:700;">' + esc(nome) + '</div></div>' +
          (cpf ? '<div style="text-align:right;"><div style="font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:#888;">CPF</div>' +
        ...
        '<div style="margin-top:40px;display:flex;justify-content:space-between;align-items:flex-end;">' +
          ...
          '<div style="text-align:center;border:2px solid ' + CORAL + ';color:' + CORAL +
            ';border-radius:8px;padding:8px 14px;transform:rotate(-4deg);">' +
      ...
      // ---------- Rodapé, com o mesmo arco do cabeçalho espelhado ----------
      '<div style="position:relative;">' +
        '<svg viewBox="0 0 720 20" preserveAspectRatio="none" style="position:absolute;left:0;top:-19px;width:100%;height:20px;display:block;">' +
          '<path d="M0,20 Q360,0 720,20 L720,0 L0,0 Z" fill="#F7F5F0"/>' +
        '</svg>' +
  ...
  function baixarPDF(rel, cliente) {
    ...
    // A logo/assinatura do contador pode ser uma foto grande rec\u00e9m-anexada:
    // sem esperar o decode, o html2canvas \u00e0s vezes rasteriza antes da imagem
    // estar pronta e ela sai em branco no PDF.
    var imgs = Array.prototype.slice.call(no.querySelectorAll('img'));
    var prontas = Promise.all(imgs.map(function (img) {
    ...
    return prontas.then(function () {
      return html2pdf().set({
        margin: 0,
        filename: nomeArq,
        // PNG em vez de JPEG: o texto do relat\u00f3rio sai n\u00edtido, sem o borr\u00e3o de
        // compress\u00e3o que o JPEG introduz em bordas finas de letras.
        image: { type: 'png' },
